...
mod tests {
    ...
    fn preserves_an_invalid_puk_for_local_rejection() {
        let value = normalized_puk_input("12x3456");
    ...
    #[test]
    fn accepts_only_eight_to_twelve_digit_puks() {
        for puk in ["49071234", "00000000", "11111111", "123456789012"] {
            let value = secret(puk.into()).expect("test PUK has valid PIN-family syntax");
            validate_gui_puk(&value).expect("eight to twelve digit PUK should be accepted");
        }
        for puk in ["490712", "4907123"] {
            let value = secret(puk.into()).expect("test PUK has valid PIN-family syntax");
            assert!(
