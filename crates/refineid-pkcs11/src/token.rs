...
fn remote_card_sign(hex_id: &str, mechanism: Mechanism, input: &[u8]) -> Result<Vec<u8>, CkRv> {
    ...
    let result = match refineid_lib_core::rapp::execute_operation_with_pair(&pair, &op) {
        Ok(res) => res,
        Err(err) => {
            crate::diag::diag!("remote_card_sign execute_operation_with_pair failed: {err:?}");
            return Err(CKR_DEVICE_ERROR);
        }
    };
            ...
            match mechanism {
                Mechanism::Ecdsa => {
                    if signature_bytes.first() == Some(&0x30) {
                        refineid_lib_core::sign::xades::ecdsa_signature_to_xml(&signature_bytes, 48)
                            .ok_or(CKR_DEVICE_ERROR)
