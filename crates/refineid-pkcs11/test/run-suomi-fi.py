...
test_pin1 = os.environ.get("REFINEID_TEST_PIN1")
...
if test_pin1:
    time.sleep(1.5)
    subprocess.run(["ydotool", "type", "--", test_pin1]) # Type PIN
    time.sleep(0.2)
    ...
    try:
        ...
        if "hst-prompt" in url:
            try:
                ...
                if test_pin1:
                    time.sleep(1)
                    subprocess.run(["ydotool", "type", "--", test_pin1])
                    time.sleep(0.2)
                    subprocess.run(["ydotool", "key", KEY_ENTER_PRESS, KEY_ENTER_RELEASE])
            except (subprocess.SubprocessError, OSError, EOFError, json.JSONDecodeError):
                pass  # transient prompt interaction failure, continue polling
