...
#[test]
fn test_request_hash_conformance() {
    let session_id = [0xbb; 16];
    let operation_id = [0x11; 16];
    let profile = "fi.refineid.authentication.v1";
    let action = "browser_authenticate";
    let mut context = BTreeMap::new();
    context.insert(
        "origin".into(),
        WireValue::Text("https://example.test".into()),
    );
    let mut payload = BTreeMap::new();
    payload.insert("digest".into(), WireValue::Bytes(vec![0xcc; 32]));
    payload.insert(
        "algorithm".into(),
        WireValue::Text("rsa_pkcs1_sha256".into()),
    );
    payload.insert("key_profile".into(), WireValue::Text("rsa_3072".into()));
    let h = derive_request_hash(
        &session_id,
        &operation_id,
        profile,
        action,
    ...
    .expect("ok");
    assert_eq!(
        crate::hex::Hex::encode(&h),
        "783b724261f793cdc13c77d8176376838c2ffaf499ec06e9bd37874706f9c0ce"
    );

    let action_cert = "read_certificate";
    let mut payload_cert = BTreeMap::new();
    payload_cert.insert("kind".into(), WireValue::Text("authentication".into()));
    let h_cert = derive_request_hash(
        &session_id,
        &operation_id,
        profile,
        action_cert,
    ...
    .expect("ok");
    assert_eq!(
        crate::hex::Hex::encode(&h_cert),
        "f4fd520ae562c6bf8c169b39f60d2d45bb76bc1ec684deed28c3aa78c1252a01"
    );
}
