...
/// Standard status profile name (`fi.refineid.card-status.v1`).
pub const PROFILE_STATUS: &str = "fi.refineid.card-status.v1";
/// Standard client authentication profile name (`fi.refineid.authentication.v1`).
pub const PROFILE_AUTH: &str = "fi.refineid.authentication.v1";
/// Standard document signature profile name (`fi.refineid.document-signing.v1`).
pub const PROFILE_SIGN: &str = "fi.refineid.document-signing.v1";
...
impl CardOperation {
    ...
    pub fn required_profile(&self) -> &'static str {
        match self {
            ...
            Self::ReadCertificate { kind } if kind == "signature" => PROFILE_SIGN,
