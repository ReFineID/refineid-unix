...
impl PairArgs {
    ...
    fn run_rendezvous_mode(self) -> ExitCode {
        ...
        let (mut stream, connected_endpoint) = match maybe_conn {
            Some(c) => c,
            None => {
        ...
        pair_record.transport_parameters.insert(
            "endpoints".into(),
            refineid_lib_core::rapp::WireValue::Array(vec![
                refineid_lib_core::rapp::WireValue::Text(connected_endpoint),
            ]),
        );
    ...
    pub fn run(self) -> ExitCode {
        ...
        let cert_der = match &pair.cached_auth_cert {
            ...
            None => {
                ...
                match execute_operation_with_pair(pair, &cert_op) {
                    Ok(CardOperationResult::Certificate { der_bytes, .. }) => {
                        let mut updated = pair.clone();
                        updated.cached_auth_cert = Some(der_bytes.clone());
                        let _ = vault.save_pair(&updated);
                        der_bytes
                    }
                    Ok(_) => {
                        eprintln!("Unexpected response while fetching certificate.");
                        return ExitCode::FAILURE;
                    }
                    Err(e) => {
